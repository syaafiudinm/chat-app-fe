...
import { useRoomStore, useMessageStore, useUIStore, useAuthStore } from '../stores';
...
export default function ChatArea() {
  ...
  const { typingUsers, toggleSidebar, toggleMembersPanel } = useUIStore();
  ...
  useEffect(() => {
    ...
    setPage(1);
    setInitialLoad(true);
    fetchMessages(activeRoomId, 1).then(() => {
    ...
    return () => {
      if (activeRoomId) {
        wsService.send('leave_room', { room_id: activeRoomId });
      }
    };
  }, [activeRoomId]);
  ...
  useEffect(() => {
    ...
    if (isNearBottom || roomMessages[roomMessages.length - 1]?.sender_id === user?.id) {
    ...
    return (
      <div className="flex-1 flex items-center justify-center bg-zinc-950">
        <div className="text-center">
          ...
          <h3 className="text-lg font-semibold text-zinc-400 mb-1">No chat selected</h3>
          <p className="text-sm text-zinc-600">Choose a conversation to start messaging</p>
        </div>
      </div>
    );
  }
          ...
          <div className="flex items-center gap-3">
            ...
            <div>
              <h2 className="text-sm font-semibold text-white flex items-center gap-1.5">
                {room.type === 'group' && <span className="text-zinc-500">#</span>}
                {room.name}
              </h2>
              {room.description && (
                <p className="text-[11px] text-zinc-600 truncate max-w-xs">{room.description}</p>
              )}
            </div>
          </div>
          ...
          {!isLoading && roomMessages.length === 0 && (
            <div className="flex flex-col items-center justify-center h-full text-center px-4">
              ...
              <p className="text-sm text-zinc-400 font-medium">
                This is the beginning of <span className="text-white">{room.name}</span>
              </p>
          ...
          {Array.from(grouped.entries()).map(([dateLabel, msgs]) => (
            <div key={dateLabel}>
              ...
              <div className="flex items-center gap-4 px-4 my-4">
                ...
                <span className="text-[11px] text-zinc-600 font-medium shrink-0">{dateLabel}</span>
                <div className="flex-1 h-px bg-zinc-800/50" />
              </div>
          ...
          {roomTyping.length > 0 && (
            <div className="px-4 py-2 flex items-center gap-2">
              ...
              <span className="text-xs text-zinc-600">
                {roomTyping.map((t) => t.username).join(', ')} {roomTyping.length > 1 ? 'are' : 'is'} typing...
              </span>
            </div>
          )}
